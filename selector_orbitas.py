...
path = glob.glob('../../../MAVEN/mag_1s/2014/10/*.sts')
cantidad_datos = len(path)
calendario_2014 = np.zeros((4, cantidad_datos)) #la primera columna es el día del año, la segunda dice si cumple el SZA, la tercera la altitud y la cuarta el ZMSO


for i,j in enumerate(path): #loop en todos los archivos .sts para cada año. i me da el índice en la lista, j me da el archivo
    mag = np.loadtxt(j, skiprows=148)

    calendario_2014[0,i] = mag[1,1]
