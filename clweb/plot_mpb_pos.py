...
"""
Este script plotea la posicion y B en el tiempo
...
axz2 = plt.subplot2grid((2, 1), (1, 0), sharex=axz1)
axz2.xaxis.set_major_formatter(xfmt)
axz2.plot(tiempo_mag, posicion)
axz2.set_ylabel("posicion (km)", fontsize=16)
axz1.tick_params(axis="both", which="major", labelsize=14)
axz2.tick_params(axis="both", which="major", labelsize=14)
axz2.set_xlabel("Time (UTC)", fontsize=16)
axz2.legend(["x", "y", "z"])
...
axz1.axvspan(
    xmin=tiempo_mag[tm_up], xmax=tiempo_mag[tm1], facecolor="#581845", alpha=0.5
)
axz1.axvspan(
    xmin=tiempo_mag[tmva], xmax=tiempo_mag[tm3], facecolor="#FFC300", alpha=0.75
)
axz1.axvspan(
    xmin=tiempo_mag[tm4], xmax=tiempo_mag[tm_down], facecolor="#C70039", alpha=0.5
)

axz2.axvspan(
    xmin=tiempo_mag[tm_up],
    xmax=tiempo_mag[tm1],
    facecolor="#581845",
    alpha=0.5,
    label="Upstream",
