...
"""
Hace el MVA para una órbita outbound. La diferencia es que cuando corre el MVA, en los returns los devuelve t4,t3,t2,t1.
...
(
    ...
    B_medio_vectorial,
    nr,
) = MVA(year, month, day, doy, ti_MVA, tf_MVA)
...
)  # Es importante que los vectores estén normalizados!
angulo_v_mva = np.arccos(np.clip(np.dot(x3, v_media_norm), -1.0, 1.0))
angulo_v_boot = np.arccos(np.clip(np.dot(normal_boot, v_media_norm), -1.0, 1.0))

B_intermedio = B_medio_vectorial / np.linalg.norm(
    B_medio_vectorial
)  # la normalizo por cuestion de  cuentas nomas
angulo_B_fit = np.arccos(np.clip(np.dot(normal_fit, B_intermedio), -1.0, 1.0))
angulo_B_mva = np.arccos(np.clip(np.dot(x3, B_intermedio), -1.0, 1.0))
angulo_B_boot = np.arccos(np.clip(np.dot(normal_boot, B_intermedio), -1.0, 1.0))
...
# los intervalos up/downstream son tomados +-54s respecto a los tiempos t1 y t4.
