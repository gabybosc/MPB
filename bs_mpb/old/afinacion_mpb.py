...
def plot_encontrar(frontera):
    ...
    while not happy:
        ...
        if max(B_norm) > 70 and B_norm[donde(t, t_mpb)] < 40:
            ax2.set_ylim([-50, 50])
            ax3.set_ylim([0, 50])
            if B_norm[donde(t, t_mpb)] < 20:
                ax2.set_ylim([-20, 20])
                ax3.set_ylim([0, 30])
        elif max(B_norm) > 70 and B_norm[donde(t, t_mpb)] > 40:
...
filepath = f"../outputs/grupo{grupo}/limites_mpb.txt"
...
with open(filepath, "a") as file:
    file.write(
        f"{cat[0]}\t{val_MPB[0]}\t{val_MPB[1]}\t{val_MPB[2]}\t{flag_MPB}\t{cat[3]}\t{cat[4]}"
    )
    file.write("\n")
