...
"""
Calcula el giroradio y la long inercial de iones. El giroradio lo calcula usando v_perp al campo B y también usando la velocidad proyectada en la normal.
"""
...
where vperp is the component of the velocity perpendicular to the direction of the magnetic field and B is the strength of the magnetic field
"""
...
rg_min = mp * min(np.linalg.norm(velocidad - N, axis=1)) / (q_e * np.linalg.norm(B_medio))
rg_max = mp * max(np.linalg.norm(velocidad - N, axis=1)) / (q_e * np.linalg.norm(B_medio))
...
print(f'La longitud inercial de iones es {ion_length:1.3g} km (entre {ion_min:1.3g} y {ion_max:1.3g})')
