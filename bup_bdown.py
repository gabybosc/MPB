...
from funciones import find_nearest, find_nearest_final, find_nearest_inicial, datenum

np.set_printoptions(precision=4)

"""
Grafica la variación en B_upstream y B_downstream en el tiempo. Sirve para elegir qué límites tomar.
"""

path = "../../datos/"  # path a los datos
datos = np.loadtxt(
    path + "MAG_1s/2016/mvn_mag_l2_2016076ss1s_20160316_v01_r01.sts", skiprows=148
...
for i in range(M - 1):
    if t[i + 1] - t[i] > 24 * 1.5e-5:  # 1.1e-5 es 1s, le doy un poco más
        print(
            "salto en la linea {} de {} segundos".format(
                i + 144, (t[i + 1] - t[i]) / (24 * 1.1e-5)
            )
        )
...
tiempo_mag = np.array(
    [np.datetime64(datenum(2016, 3, 16, x)) for x in t]
