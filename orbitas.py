...
calendario = np.zeros((5 * cantidad_datos, 6)) #la primera columna es el día del año, la segunda es el número de orbita, la tercera dice si cumple el SZA, la cuarta la altitud y la quinta el ZMSO
...
np.savetxt('SZA30_{}.txt'.format(year), sorted(fechas_SZA30_trim, key=lambda x: x[0]), fmt='%10d' ,header= "Las fechas de {} en las cuales tenemos SZA < 30º".format(year), newline="\r\n")

np.savetxt('SZA60_{}.txt'.format(year), sorted(fechas_SZA60_trim, key=lambda x: x[0]), fmt='%10d' ,header= "Las fechas de {} en las cuales tenemos SZA > 30º y < 60º".format(year), newline="\r\n")

np.savetxt('SZA90_{}.txt'.format(year), sorted(fechas_SZA90_trim, key=lambda x: x[0]), fmt='%10d' ,header= "Las fechas de {} en las cuales tenemos SZA > 60º".format(year), newline="\r\n")
