...
calendario_2017 = np.zeros(
    (5, cantidad_datos)
)  # la primera columna es el día del año, la segunda es el número de orbita, la tercera dice si cumple el SZA, la cuarta la altitud y la quinta el ZMSO
    ...
    for indice, l in enumerate(orbitas):
        ...
        SZA = (
            np.arccos(
                np.clip(
                    np.dot(
                        pos[int(idx)] / np.linalg.norm(posicion[int(idx)]), [1, 0, 0]
                    ),
                    -1.0,
                    1.0,
                )
