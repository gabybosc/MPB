...
for j in range(len(fechas)):
    ...
    #     f"../../../datos/SWEA/mvn_swe_l2_svyspec_{year}{month}{day}_v04_r01.cdf", "wb"
    # ) as out_file:
    #     shutil.copyfileobj(response, out_file)
    # print(f"swea dia {doy} listo")

    with urllib.request.urlopen(swia_mom) as response, open(
        f"../../datos/SWIA/mvn_swi_l2_onboardsvymom_{year}{month}{day}_v01_r01.cdf",
        "wb",
    ) as out_file:
        shutil.copyfileobj(response, out_file)
    ...
    #     f"../../../datos/SWIA/mvn_swi_l2_coarsearc3d_{year}{month}{day}_v01_r01.cdf",
    #     "wb",
    # ) as out_file:
    #     shutil.copyfileobj(response, out_file)
