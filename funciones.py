...
def Mij(B):
    ...
    for i in range(3):  # para las tres coordenadas
        for j in range(3):
            M_ij[i,j] = np.mean(B[:,i] * B[:,j]) - np.mean(B[:,i]) * np.mean(B[:,j])
    return M_ij
...
def datenum(Yr,Mo=1,Da=1,Hr=0,Mi=0,Se=0,Ms=0):
    ...
    mytime = dt.datetime(int(Yr),int(Mo),int(Da))+dt.timedelta(days=extraDa+getrem(Da),
                                                               hours=Hr,minutes=Mi,seconds=Se,microseconds=Ms)
    return mytime
